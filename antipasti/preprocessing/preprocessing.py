    ...
    def generate_maps(self):
        ...
        for i, entry in enumerate(self.entries):
            ...
            if not self.cmaps: # and len(np.load(self.residues_path + path[-11:-7] + '.npy')) > 500:
                ...
                # subprocess.call(['/usr/local/bin/RScript', str(self.scripts_path)+'pdb_to_dccm.r', str(path), str(new_path), str(self.modes)], stdout=open(os.devnull, 'wb')) # added to make dccm map is generated
            #elif not self.cmaps:
            #    print(path[-11:-7])
